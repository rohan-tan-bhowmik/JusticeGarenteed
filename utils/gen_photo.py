...
def main():
    ...
    for i in tqdm(range(100)):
        ...
        map_imgs = [os.path.join('../greenscreends/train/', img) for img in all_imgs if img.startswith('map_frame_')]
        # Separate by role
        champ_imgs = [os.path.join('../greenscreends/train/', img)
                    for img in all_imgs if not img.startswith(('red-','blue-', 'map_frame_'))]
        # champ_imgs = ["/Users/HP/Documents/GitHub/JusticeGarenteed/greenscreends/train/aatrox_train_00013_png.rf.5c73c8fb22c9a17024f048c6a90db6e9.jpg"]
        ...
        # num_champions = 1
        # Minions have mean 5 std 7 clamp to larger than 0
        num_minions = int(np.clip(random.gauss(5, 7), 1, 20))
        ...
        #     # High opacity champions, occasional ghosty ones
        #     if random.random() < 0.9:
        #         t = 1.0
        #     else:
        #         t = np.clip(random.gauss(0.6, 0.1), 0, 1)

        #     alpha = (mask * t * 255).astype(np.uint8)

        #     # Premultiply RGB
        #     alpha_f = alpha[..., None] / 255.0
        #     rgb = (rgb * alpha_f).astype(np.uint8)
        #     rgba = np.dstack([rgb, alpha])
        ...
        # Apply light color jitter to the final result (after all blending)
        # final_rgba = np.array(pil_composed)
        # rgb = final_rgba[..., :3]
        # alpha = final_rgba[..., 3]
        ...
        # pil_composed = Image.fromarray(final_rgba, mode="RGBA")
        ...
        # Add health bars
        font_path = '../BeaufortForLoL-OTF/BeaufortforLOL-Bold.otf'
        ...
        cv2.imwrite(f'../test_images/map_{i:04d}.jpg', map_img)
