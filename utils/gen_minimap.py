...
class ImageDrawer:
    ...
    def get_minimap(self):
        """
        Generates and returns a synthetic minimap image (torch.Tensor)
        without any COCO metadata.
        """
        # you can choose any integer here; it's only used for image_id in your annotations
        img, _ = self.generate_data(index=0)
        return img

    def get_minimap_np(self):
        """
        Same as get_minimap, but returns a NumPy array (H×W×C, values in [0,1]).
        """
        img = self.get_minimap()               # torch.Tensor, C×H×W
