...
def process_healthbar_template(img: np.ndarray,
    ...
    """
    Assumes BGR image with healthbar template.
    """
    ...
    if random.random() < damage_prob:
        ...
        damage_w = 0
    health_w = int((total_w - damage_w )* (1 - (random.randint(0, total_w - damage_w) / (total_w - damage_w))**2))
    h_end = x1 + health_w
    d_end = h_end + damage_w

    # draw sections
    img[y1:y2, x1:h_end]    = orig[y1:y2, x1:h_end]      # healthy (blue or red)
    img[y1:y2, h_end:d_end] = [33, 193, 232]             # recent damage orangey-yellow
